/*
Lunastamps jQuery (https://github.com/openluna/lunastamp)
jquery.lunastamp.js
Version 0.0.1
    ...
    init: function(callback) {
      ...
      stampDiv.on('touchstart', function(event) {
        ...
        if (event.originalEvent.touches.length === 5) {
          ...
          for (var i = 0; i < 4; i++) {
            for (var j = i + 1; j < 5; j++) {
              var dis = vectorDistance(touches[i], touches[j])
              if (dis > max_distance) {
                max_distance = dis;
      ...
      function pointInLineNearestToPoint(a, b, c) {
        var B = vectorDistance(a, c)
        var A = distanceFromLine(a, b, c)
      ...
      function getStampName(stamp) {
        ...
        for (var i = 0; i < stamps.length; i++) {
          var norm = stamp.max_distance / stamps[i].distance
          var text = text + "max_distance: " + stamp.max_distance + "<br>"
